...
class FossbytesCom extends Parser
{
    public $contentSelector = '.td-main-content > div > div.td-post-content.td-pb-padding-side';
    ...
    public function filterUrl(&$links = []){
        foreach ($links as $key => $value){
            if(preg_match('|^https://fossbytes.com/category/how-to|', $this->currentUrl)){
                if(preg_match('|^https://fossbytes.com/[\w-]+|', $value)){
                    continue;
                }
            }
            if(!preg_match('|^https://fossbytes.com/category/how-to/|', $value)){
                unset($links[$key]);
            }
        }
        print_r($links);
    ...
    public function parseArticle($document)
    {
        ...
        $activeObjects = $contentDom->find('blockquote');
        foreach ($activeObjects as $elem) {
            $pq = pq($elem);
            $pq->remove();
        }

        $activeObjects = $contentDom->find('.code-block');
        foreach ($activeObjects as $elem) {
            $pq = pq($elem);
            $pq->remove();
        }

        $activeObjects = $contentDom->find('meta');
        foreach ($activeObjects as $elem) {
            $pq = pq($elem);
            $pq->remove();
        }
        $activeObjects = $contentDom->find('span[itemscope]');
        foreach ($activeObjects as $elem) {
            $pq = pq($elem);
            $pq->remove();
        }
        ...
        $thumbSrc = null;
        if(count($document->find('div.td-post-featured-image > a > img'))){
            $thumbs = $document->find('div.td-post-featured-image > a > img');
